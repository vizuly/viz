...
vizuly2.viz.ColumnChart = function (parent) {
	...
	function measure() {
		...
		if (typeof scope.xScale.clamp != 'undefined') {
			seriesScalePadding = (size.width / scope.data[0].length) - barPadding;
		}
		
		scope.xScale.range([0, ((groupWidth + groupPadding) * scope.data[0].length) - seriesScalePadding]);
	...
	function update() {
		...
		bottomAxis.attr("transform", "translate(" + (size.left + seriesScalePadding/2 ) + "," + (scope.height - (scope.height - size.height)/2) + ")");
		...
		barGroup.each(function (datum, index) {
			...
			group.attr("transform", function (d, i) {
				return "translate(" + (scope.xScale(scope.x(datum.data['series0'].data)) + groupPadding/2) +  ",0)"
			});
